async function getDataPelanggan() {
    ...
    hasil.then(data=>{
        ...
        data.forEach((result) => {
            if(result.status == "PROSES" && result.paket == "HIGH"){
                count_1 += 1;
                show.innerHTML +=
                    `<tr class="isi-data">
                        <td>${result.id}</td>
                    ...
                    } else if(result.tipe_pengantaran == "Diantar & Dijemput"){
                        show.innerHTML =
                            show.innerHTML +
                            `<tr class="hide">
                                <td style="text-align:left;" colspan="8">Estimasi Selesai : ${result.tanggal} <br> Waktu Pengantaran : ${result.waktu_pengantaran} <br> Alamat Pengantaran : ${result.alamat_pengantaran} <br> Waktu Penjemputan : ${result.waktu_penjemputan} <br> Alamat Penjemputan : ${result.alamat_penjemputan}</td>
                            </tr>`;
                    } else{
                        show.innerHTML += 
                        `<tr class="hide">
            ...
            } else if(result.status == "SELESAI" && result.paket == "HIGH"){
                count_2 += 1;
                show2.innerHTML +=
                `<tr class="isi-data2">
                    <td>${result.id}</td>
                ...
                if(result.tipe_pengantaran == "Diantar"){
                    ...
                    show2.innerHTML =
                        show2.innerHTML +
                        `<tr class="hide">
                            <td style="text-align:left;" colspan="8">Estimasi Selesai : ${result.tanggal} <br> Waktu Pengantaran : ${result.waktu_pengantaran} <br> Alamat Pengantaran : ${result.alamat_pengantaran} <br> Waktu Penjemputan : ${result.waktu_penjemputan} <br> Alamat Penjemputan : ${result.alamat_penjemputan} <br> Tanggal Selesai : ${result.tanggal_selesai}</td>
                        </tr>`;
                } else{
                    show2.innerHTML += 
                        `<tr class="hide">
